...
const COURSES = {
  ...
  day2: { name: "Day 2 Course (TBD)", par: Array(18).fill(4), hcp: Array.from({length:18},(_,i)=>i+1) },
  day3: { name: "Sunday Singles (TBD)", par: Array(18).fill(4), hcp: Array.from({length:18},(_,i)=>i+1) },
};
...
const initialDays = [
  ...
  {
    day:2, courseKey:"day2", label:"Day 2 — Foursomes", format:"Foursomes",
    matches: Array.from({length:5},(_,i)=>({id:200+i+1,teeTime:"TBD",player1a:`Player A${i*2+1}`,hcp1a:0,player1b:`Player A${i*2+2}`,hcp1b:0,player2a:`Player B${i*2+1}`,hcp2a:0,player2b:`Player B${i*2+2}`,hcp2b:0,scores:mkScores()}))
  },
  {
    day:3, courseKey:"day3", label:"Sunday Singles", format:"Singles",
    matches: Array.from({length:9},(_,i)=>({id:300+i+1,teeTime:"TBD",player1a:`Player A${i+1}`,hcp1a:0,player1b:null,hcp1b:0,player2a:`Player B${i+1}`,hcp2a:0,player2b:null,hcp2b:0,scores:mkScores()}))
  }
];
...
const CBS_LOGO = (
  <svg width="20" height="22" viewBox="0 0 220 240" xmlns="http://www.w3.org/2000/svg">
    <polygon points="105,5 15,145 55,145 105,70 105,5" fill="#2878be"/>
    <polygon points="100,5 130,5 130,235 118,245 105,235 100,235" fill="#2878be"/>
...
const CBS_LOGO_LG = (
  <svg width="44" height="48" viewBox="0 0 220 240" xmlns="http://www.w3.org/2000/svg">
    <polygon points="105,5 15,145 55,145 105,70 105,5" fill="#2878be"/>
    <polygon points="100,5 130,5 130,235 118,245 105,235 100,235" fill="#2878be"/>
    <polygon points="15,150 210,150 225,165 210,180 15,180" fill="#2878be"/>
    <polygon points="40,188 195,188 210,200 195,212 40,212" fill="#2878be"/>
      ...
      <div style={{background:CARD,borderBottom:`1px solid ${BORDER}`,padding:"10px 12px",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",alignItems:"stretch",borderRadius:8,overflow:"hidden",border:`1px solid ${BORDER}`}}>
          ...
          <div style={{background:runLeader==="A"?TEAM_A_COLOR:runLeader==="B"?TEAM_B_COLOR:"#1a2a44",minWidth:68,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"6px 4px",borderLeft:`1px solid ${BORDER}`,borderRight:`1px solid ${BORDER}`,flexShrink:0}}>
            ...
            :<><div style={{fontSize:17,fontWeight:900,color:"#fff",fontFamily:"monospace",lineHeight:1}}>{runLeader?runAbs:"AS"}</div><div style={{fontSize:8,color:"#88aacc",fontFamily:"monospace",marginTop:1}}>{runLeader?"UP":"ALL SQ"}</div><div style={{fontSize:8,color:"#446",fontFamily:"monospace",marginTop:2}}>H{hole+1}</div></>}
          </div>
...
function TVMatchRow({ match, isSingles, onOpen }) {
  ...
  else if(live&&!s.leader){centreTop=`H${s.holesPlayed}`;centreBot="AS";centreBg="#1a2a44";}
  else if(live){centreTop=`H${s.holesPlayed}`;centreBot=`${s.up}UP`;centreBg=s.leader==="A"?`${TEAM_A_COLOR}cc`:`${TEAM_B_COLOR}cc`;}
