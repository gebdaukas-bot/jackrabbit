  ...
  const Row = ({ label, field, color }) => (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${BORDER}`}}>
      ...
      <div style={{display:"flex",alignItems:"center"}}>
        <button onClick={()=>setVals(v=>({...v,[field]:Math.round(Math.max(-10,v[field]-0.1)*10)/10}))} style={{width:32,height:34,fontSize:18,background:CARD2,border:`1px solid ${BORDER}`,borderRadius:"6px 0 0 6px",color:"#8aa",cursor:"pointer"}}>−</button>
        ...
        <button onClick={()=>setVals(v=>({...v,[field]:Math.round(Math.min(36,v[field]+0.1)*10)/10}))} style={{width:32,height:34,fontSize:18,background:CARD2,border:`1px solid ${BORDER}`,borderRadius:"0 6px 6px 0",color:"#8aa",cursor:"pointer"}}>+</button>
      </div>
    </div>
  );
